...
(function () {
  ...
  function analyzeGithub(target, meta, commits, tags, releases) {
    var list = commits.map(function (c) {
      ...
      var a = cm.author || {
        ...
        date: (cm.committer && cm.committer.date) || (c.author && c.author.date),
        message: ""
      };
      return {
        author: (c.author && c.author.login) ? c.author.login : (a.name || "unknown"),
    ...
    return {
      ...
      totals: totals,
      shape: detectShape(monthList, n, ageDays),
      months: monthList,
  ...
  function detectShape(months, n, ageDays) {
    if (months.length <= 1 || n < 5 || (n < 60 && ageDays < 90)) return { arc: "fresh", label: "Fresh Start", summary: n + " commits and counting — this story is just opening." };
    ...
    api(API + "/repos/" + target.owner + "/" + target.repo)
      ...
      .catch(function (err) {
        ...
        if (err instanceof UserError) showError(err.message, err.hint);
          ...
          console.error("[commit-canvas] analysis failed:", err);
          var net = wrapNet(err);
