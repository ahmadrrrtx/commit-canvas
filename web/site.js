...
(function () {
  ...
  /* ── live GitHub star count (graceful: plain label if unreachable) ── */
  $$("a[data-star]").forEach(function (a) {
    if (typeof fetch !== "function") return;   /* keep plain label */
    fetch("https://api.github.com/repos/ahmadrrrtx/commit-canvas")
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (j) {
        if (!j || typeof j.stargazers_count !== "number") return;
        var n = j.stargazers_count >= 1000
          ? (j.stargazers_count / 1000).toFixed(1).replace(/\.0$/, "") + "k" : String(j.stargazers_count);
        a.textContent = "★ " + n + " on GitHub";
        a.setAttribute("aria-label", "Star commit-canvas on GitHub — " + j.stargazers_count + " stars");
      })
      .catch(function () { /* keep plain label */ });
  });
